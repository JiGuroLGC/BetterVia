...
public class Hook implements IXposedHookLoadPackage {
	...
	private static boolean isOnlinePreviewSwitchChanging = false;
	...
	private static final String GITHUB_THEMES_JSON_URL = "https://raw.githubusercontent.com/JiGuroLGC/CDN/main/themes_new.json";
	private static final String GITHUB_UPDATE_URL = "https://raw.githubusercontent.com/JiGuroLGC/CDN/main/update.json";
	...
	private static final String GITHUB_SHISUI_JSON_URL = "https://raw.githubusercontent.com/JiGuroLGC/CDN/main/shisui.json";

	public static final String VERCEL_MAPPING_JSON_URL = "https://raw.196104.xyz/mapping.json";
	public static final String GITHUB_MAPPING_JSON_URL = "https://raw.githubusercontent.com/JiGuroLGC/CDN/main/mapping.json";
	...
	private void handleViaApp(final XC_LoadPackage.LoadPackageParam param) {
		XposedHelpers.findAndHookMethod(Application.class, "attach", Context.class, new XC_MethodHook() {
			...
			protected void afterHookedMethod(MethodHookParam attachParam) throws Throwable {
						...
						new Handler(Looper.getMainLooper()).post(new Runnable() {
							...
							public void run() {
								try {
									...
									String testClass = ViaClassMapping
				...
				boolean needClearNetworkMappings = getPrefBoolean(ctx, KEY_NEED_CLEAR_NETWORK_MAPPINGS, false);
				boolean skipClearNetworkMappings = getPrefBoolean(ctx, KEY_SKIP_CLEAR_NETWORK_MAPPINGS, false);
				...
				if (hasLanguageSelection && hasValidNetworkSource && hasUserAgreement && hasVersionSelection) {
					...
					try {
						...
						XposedHelpers.findAndHookMethod(privacyLockOverlayClass, cl, privacyLockOverlayMethod,
	...
	private void showSettingsDialog(final Context ctx) {
		...
		act.runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				subtitle.setPadding(0, 0, 0, dp(act, 12));
				root.addView(subtitle);

				addCategoryDivider(root, ctx, "category_basic");
				...
				addCategoryDivider(root, ctx, "category_appearance");
				...
				monetManager.addMonetMomentItem(root, act, ctx);
				...
				addCategoryDivider(root, ctx, "category_privacy");
				...
				addCategoryDivider(root, ctx, "category_playback");

				addSwitch(root, act, LocalizedStringProvider.getInstance().get(ctx, "long_press_speed_switch"),
						LocalizedStringProvider.getInstance().get(ctx, "long_press_speed_hint"), KEY_LONG_PRESS_SPEED,
						false, new Runnable() {
							@Override
							public void run() {
								boolean on = getPrefBoolean(ctx, KEY_LONG_PRESS_SPEED, false);
								VideoPlayerHook.initVideoPlayerHook(ctx, act.getClassLoader());
							}
						});
			
				addSwitch(root, act, LocalizedStringProvider.getInstance().get(ctx, "free_zoom_switch"),
						LocalizedStringProvider.getInstance().get(ctx, "free_zoom_hint"), KEY_FREE_ZOOM, false,
						new Runnable() {
							@Override
							public void run() {
								boolean on = getPrefBoolean(ctx, KEY_FREE_ZOOM, false);
								VideoPlayerHook.initVideoPlayerHook(ctx, act.getClassLoader());
							}
						});

				addCategoryDivider(root, ctx, "category_repository");
				...
				addCategoryDivider(root, ctx, "category_other");
				...
				addOnlinePreviewItem(root, act, ctx);

				addCategoryDivider(root, ctx, "category_module");

				LinearLayout versionContainer = new LinearLayout(act);
				versionContainer.setOrientation(LinearLayout.VERTICAL);
				versionContainer.setPadding(0, dp(act, 8), 0, dp(act, 8));
				...
				versionContainer.addView(versionRow);
				...
				versionHintTv.setPadding(0, dp(act, 4), 0, 0);
				versionContainer.addView(versionHintTv);
					...
					public void onClick(View v) {
						...
						list.setOnItemClickListener(new AdapterView.OnItemClickListener() {
							...
							public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
								...
								bvLog("[BetterVia] 用户在设置中选择的Via版本: " + supportedVersionNamesList.get(position)
				...
				LinearLayout langContainer = new LinearLayout(act);
				langContainer.setOrientation(LinearLayout.VERTICAL);
				langContainer.setPadding(0, dp(act, 8), 0, dp(act, 8));
				...
				langHintTv.setPadding(0, dp(act, 4), 0, 0);
				langContainer.addView(langHintTv);
				root.addView(langContainer);

				LinearLayout sourceContainer = new LinearLayout(act);
				sourceContainer.setOrientation(LinearLayout.VERTICAL);
				sourceContainer.setPadding(0, dp(act, 8), 0, dp(act, 8));
				...
				final String[] sourceItems = new String[]{"Vercel", "GitHub"};
				...
				sourceContainer.addView(sourceRow);
				...
				sourceHintTv.setPadding(0, dp(act, 4), 0, 0);
				sourceContainer.addView(sourceHintTv);
				...
				LinearLayout mappingSyncContainer = new LinearLayout(act);
				mappingSyncContainer.setOrientation(LinearLayout.VERTICAL);
				mappingSyncContainer.setPadding(0, dp(act, 8), 0, dp(act, 8));
				...
				mappingSyncContainer.addView(mappingSyncRow);
				...
				mappingSyncHintTv.setPadding(0, dp(act, 4), 0, 0);
				mappingSyncContainer.addView(mappingSyncHintTv);
				root.addView(mappingSyncContainer);
				...
				mappingSyncButton.setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View v) {
						...
						mappingSyncButton
								.setText(LocalizedStringProvider.getInstance().get(ctx, "mapping_sync_loading"));
						...
						MappingSyncHelper.syncMappingFromNetwork(ctx, new MappingSyncHelper.SyncCallback() {
							...
							public void onSuccess(int addedVersions) {
								...
								mappingSyncButton
										.setText(LocalizedStringProvider.getInstance().get(ctx, "mapping_sync_button"));
				...
				LinearLayout themeContainer = new LinearLayout(act);
				themeContainer.setOrientation(LinearLayout.VERTICAL);
				themeContainer.setPadding(0, dp(act, 8), 0, dp(act, 8));
				...
				themeHintTv.setPadding(0, dp(act, 4), 0, 0);
				themeContainer.addView(themeHintTv);
				root.addView(themeContainer);
				...
				LinearLayout aboutContainer = new LinearLayout(act);
				aboutContainer.setOrientation(LinearLayout.VERTICAL);
				aboutContainer.setPadding(0, dp(act, 8), 0, dp(act, 8));
				...
				aboutBtn.setOnClickListener(new View.OnClickListener() {
					@Override
					public void onClick(View v) {
						showAboutDialog(ctx, dialogRef[0]);
					}
				});
				aboutRow.addView(aboutBtn);

				aboutContainer.addView(aboutRow);
				...
				aboutHintTv.setPadding(0, dp(act, 4), 0, 0);
				aboutContainer.addView(aboutHintTv);
				root.addView(aboutContainer);

				LinearLayout storageContainer = new LinearLayout(act);
				storageContainer.setOrientation(LinearLayout.VERTICAL);
				storageContainer.setPadding(0, dp(act, 8), 0, dp(act, 8));
				...
				storageContainer.addView(storageRow);
				...
				storageHintTv.setPadding(0, dp(act, 4), 0, 0);
				storageContainer.addView(storageHintTv);
				root.addView(storageContainer);

				LinearLayout shisuiContainer = new LinearLayout(act);
				shisuiContainer.setOrientation(LinearLayout.VERTICAL);
				shisuiContainer.setPadding(0, dp(act, 8), 0, dp(act, 8));
				...
				shisuiContainer.addView(shisuiRow);
				...
				shisuiHintTv.setPadding(0, dp(act, 4), 0, 0);
				shisuiContainer.addView(shisuiHintTv);
				root.addView(shisuiContainer);
	...
	private void addCategoryDivider(LinearLayout parent, Context ctx, String categoryKey) {
	
		TextView categoryTitle = new TextView(ctx);
		categoryTitle.setText(LocalizedStringProvider.getInstance().get(ctx, categoryKey));
		categoryTitle.setTextSize(TypedValue.COMPLEX_UNIT_SP, 14);
		categoryTitle.setTextColor(getHintColor(ctx));
		categoryTitle.setTypeface(null, Typeface.BOLD);
		categoryTitle.setPadding(0, dp(ctx, 16), 0, dp(ctx, 4));
		parent.addView(categoryTitle);

		View divider = new View(ctx);
		LinearLayout.LayoutParams dividerLp = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
				dp(ctx, 1));
		dividerLp.setMargins(0, 0, 0, dp(ctx, 8));
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					...
					String jsonUrl = networkSource.equals(NETWORK_SOURCE_VERCEL)
			...
			public void run() {
				try {
					String jsCode = "javascript:(function() {" + " // 阻止页面可见性变化导致的视频暂停"
							...
							+ "    var originalVisibilityState = document.visibilityState;" + "    " + " // 重写页面可见性属性"
	...
	private void loadRulesFromNetwork(final Context ctx, final RulesLoadCallback callback) {
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					String networkSource = getPrefString(ctx, KEY_NETWORK_SOURCE, DEFAULT_NETWORK_SOURCE);
					String rulesUrl = networkSource.equals(NETWORK_SOURCE_VERCEL)
					...
					public void onClick(View v) {
						...
						act.startActivityForResult(
								Intent.createChooser(i,
										LocalizedStringProvider.getInstance().get(ctx, "homepage_bg_pick_title")),
								0x1002);
					}
				});
	...
	private boolean isViaDownloadDialog(Dialog dialog) {
		try {
			...
			int copyLinkId = ViaClassMapping.getResourceId(ViaClassMapping.ResourceKey.DOWNLOAD_DIALOG_COPY_LINK_BUTTON,
					ctx);
			int cancelId = ViaClassMapping.getResourceId(ViaClassMapping.ResourceKey.DOWNLOAD_DIALOG_CANCEL_BUTTON,
					ctx);
			int okId = ViaClassMapping.getResourceId(ViaClassMapping.ResourceKey.DOWNLOAD_DIALOG_OK_BUTTON, ctx);
	...
	private TextView createShareButton(Context ctx, Dialog dialog) {
		try {
			...
			int copyLinkId = ViaClassMapping.getResourceId(ViaClassMapping.ResourceKey.DOWNLOAD_DIALOG_COPY_LINK_BUTTON,
					ctx);
			int shareButtonId = ViaClassMapping.getResourceId(ViaClassMapping.ResourceKey.DOWNLOAD_DIALOG_SHARE_BUTTON,
					ctx);
	...
	private void addUserSandboxItem(LinearLayout parent, final Activity act, final Context ctx) {
		LinearLayout container = new LinearLayout(ctx);
		container.setOrientation(LinearLayout.VERTICAL);
		container.setPadding(0, dp(ctx, 8), 0, dp(ctx, 8));
		...
		TextView tv = new TextView(ctx);
		tv.setText(LocalizedStringProvider.getInstance().get(ctx, "user_sandbox_title"));
		tv.setTextSize(TypedValue.COMPLEX_UNIT_SP, 16);
		tv.setTextColor(getTextColor(ctx));
		hor.addView(tv, new LinearLayout.LayoutParams(0, ViewGroup.LayoutParams.WRAP_CONTENT, 1));
		...
		TextView configBtn = new TextView(ctx);
		applyClickAnim(configBtn);
		configBtn.setText(LocalizedStringProvider.getInstance().get(ctx, "user_sandbox_config"));
		configBtn.setTextSize(TypedValue.COMPLEX_UNIT_SP, 14);
		configBtn.setPadding(dp(ctx, 12), dp(ctx, 6), dp(ctx, 12), dp(ctx, 6));
		configBtn.setBackground(getRoundBg(ctx, getBtnBgColor(ctx), 8));
		configBtn.setTextColor(getBtnTextColor(ctx));
		...
		TextView hintTv = new TextView(ctx);
		hintTv.setText(LocalizedStringProvider.getInstance().get(ctx, "user_sandbox_hint"));
		hintTv.setTextSize(TypedValue.COMPLEX_UNIT_SP, 12);
		hintTv.setTextColor(getHintColor(ctx));
		hintTv.setPadding(0, dp(ctx, 4), 0, 0);
	...
	private void addOnlinePreviewItem(LinearLayout parent, final Activity act, final Context ctx) {
		...
		TextView tv = new TextView(ctx);
		tv.setText(LocalizedStringProvider.getInstance().get(ctx, "online_preview_title"));
		tv.setTextSize(TypedValue.COMPLEX_UNIT_SP, 16);
		tv.setTextColor(getTextColor(ctx));
		hor.addView(tv, new LinearLayout.LayoutParams(0, ViewGroup.LayoutParams.WRAP_CONTENT, 1));
		...
		TextView configBtn = new TextView(ctx);
		applyClickAnim(configBtn);
		configBtn.setText(LocalizedStringProvider.getInstance().get(ctx, "online_preview_config"));
		configBtn.setTextSize(TypedValue.COMPLEX_UNIT_SP, 14);
		configBtn.setPadding(dp(ctx, 12), dp(ctx, 6), dp(ctx, 12), dp(ctx, 6));
		configBtn.setBackground(getRoundBg(ctx, getBtnBgColor(ctx), 8));
		configBtn.setTextColor(getBtnTextColor(ctx));
		...
		TextView hintTv = new TextView(ctx);
		hintTv.setText(LocalizedStringProvider.getInstance().get(ctx, "online_preview_hint"));
		hintTv.setTextSize(TypedValue.COMPLEX_UNIT_SP, 12);
		hintTv.setTextColor(getHintColor(ctx));
		hintTv.setPadding(0, dp(ctx, 4), 0, 0);
	...
	private void showUserSandboxDialog(final Context ctx) {
		...
		act.runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				TextView title = new TextView(act);
				title.setText(LocalizedStringProvider.getInstance().get(ctx, "user_sandbox_dialog_title"));
				...
				TextView subtitle = new TextView(act);
				subtitle.setText(LocalizedStringProvider.getInstance().get(ctx, "user_sandbox_dialog_subtitle"));
				...
				enableTitle.setText(LocalizedStringProvider.getInstance().get(ctx, "user_sandbox_enable"));
				...
				final Switch sandboxSwitch = new Switch(act);
				sandboxSwitch.setText(LocalizedStringProvider.getInstance().get(ctx, "user_sandbox_enable"));
				sandboxSwitch.setTextSize(TypedValue.COMPLEX_UNIT_SP, 15);
				sandboxSwitch.setTextColor(textColor);
				sandboxSwitch.setGravity(Gravity.START | Gravity.CENTER_VERTICAL);
				sandboxSwitch.setChecked(getPrefBoolean(ctx, KEY_USER_SANDBOX_ENABLE, false));
				if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
					int[][] states = new int[][]{new int[]{android.R.attr.state_checked},
							new int[]{-android.R.attr.state_checked}};
					...
					if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
						sandboxSwitch.setThumbTintList(colorStateList);
					}
					if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
						sandboxSwitch.setTrackTintList(colorStateList);
					}
				}
				...
				enableContainer.addView(sandboxSwitch, switchLp);

				TextView switchHint = new TextView(act);
				switchHint.setText(LocalizedStringProvider.getInstance().get(ctx, "user_sandbox_enable_hint"));
				...
				notesTitle.setText(LocalizedStringProvider.getInstance().get(ctx, "user_sandbox_notes_title"));
				...
				notesContent.setText(LocalizedStringProvider.getInstance().get(ctx, "user_sandbox_notes_content"));
				...
				configTitle.setText(LocalizedStringProvider.getInstance().get(ctx, "user_sandbox_config_section"));
				...
				LinearLayout hideDownloadContainer = createSandboxCheckbox(act, ctx,
						LocalizedStringProvider.getInstance().get(ctx, "user_sandbox_hide_download"),
						LocalizedStringProvider.getInstance().get(ctx, "user_sandbox_hide_download_hint"),
						KEY_USER_SANDBOX_HIDE_DOWNLOAD, textColor, hintColor, switchOnColor, switchOffColor);
				final CheckBox hideDownloadCb = (CheckBox) hideDownloadContainer.getChildAt(0);
				LinearLayout.LayoutParams cbLp = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
						ViewGroup.LayoutParams.WRAP_CONTENT);
				cbLp.bottomMargin = dp(act, 8);
				configInner.addView(hideDownloadContainer, cbLp);

				LinearLayout hideCacheContainer = createSandboxCheckbox(act, ctx,
						LocalizedStringProvider.getInstance().get(ctx, "user_sandbox_hide_cache"),
						LocalizedStringProvider.getInstance().get(ctx, "user_sandbox_hide_cache_hint"),
						KEY_USER_SANDBOX_HIDE_CACHE, textColor, hintColor, switchOnColor, switchOffColor);
				final CheckBox hideCacheCb = (CheckBox) hideCacheContainer.getChildAt(0);
				configInner.addView(hideCacheContainer);
				...
				Button forceRestore = new Button(act);
				applyClickAnim(forceRestore);
				forceRestore.setText(LocalizedStringProvider.getInstance().get(ctx, "user_sandbox_force_restore"));
				forceRestore.setTextColor(textColor);
				forceRestore.setTextSize(TypedValue.COMPLEX_UNIT_SP, 14);
				forceRestore.setTypeface(null, Typeface.BOLD);
				GradientDrawable forceBtnBg = new GradientDrawable();
				forceBtnBg.setColor(btnBgColor);
				forceBtnBg.setCornerRadius(dp(act, 12));
				forceRestore.setBackground(forceBtnBg);
				LinearLayout.LayoutParams forceLp = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
						ViewGroup.LayoutParams.WRAP_CONTENT);
				forceLp.topMargin = dp(act, 16);
				root.addView(forceRestore, forceLp);
				...
				final Runnable updateConfigLock = new Runnable() {
					@Override
					public void run() {
						boolean sandboxEnabled = sandboxSwitch.isChecked();
						if (sandboxEnabled) {
							FrameLayout overlay = new FrameLayout(act);
							GradientDrawable overlayBg = new GradientDrawable();
							overlayBg.setColor(0x88000000);
							overlayBg.setCornerRadius(dp(act, 12));
							overlay.setBackground(overlayBg);
							FrameLayout.LayoutParams overlayLp = new FrameLayout.LayoutParams(
									ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);
							configContainer.addView(overlay, overlayLp);
							hideDownloadCb.setEnabled(false);
							hideCacheCb.setEnabled(false);
						} else {
							if (configContainer.getChildCount() > 1) {
								configContainer.removeViewAt(1);
							}
							hideDownloadCb.setEnabled(true);
							hideCacheCb.setEnabled(true);
						}
					}
				};

				sandboxSwitch.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
					@Override
					public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
						act.runOnUiThread(updateConfigLock);
					}
				});

				act.runOnUiThread(updateConfigLock);

				forceRestore.setOnClickListener(new View.OnClickListener() {
					@Override
					public void onClick(View v) {
						if (isProcessing[0]) {
							return;
				...
				ok.setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View v) {
						...
						boolean enable = sandboxSwitch.isChecked();
						boolean hideDownload = hideDownloadCb.isChecked();
						boolean hideCache = hideCacheCb.isChecked();

						if (!hideDownload && !hideCache) {
							jiguroMessageWithContext(ctx,
									LocalizedStringProvider.getInstance().get(ctx, "user_sandbox_select_one"));
							return;
						}

						putPrefBoolean(ctx, KEY_USER_SANDBOX_ENABLE, enable);
						putPrefBoolean(ctx, KEY_USER_SANDBOX_HIDE_DOWNLOAD, hideDownload);
						putPrefBoolean(ctx, KEY_USER_SANDBOX_HIDE_CACHE, hideCache);

						dialog.dismiss();
						performSandboxOperation(ctx, enable, false);
					}
				});
	...
	private void showOnlinePreviewDialog(final Context ctx) {
		...
		act.runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				TextView title = new TextView(act);
				title.setText(LocalizedStringProvider.getInstance().get(ctx, "online_preview_dialog_title"));
				title.setTextSize(TypedValue.COMPLEX_UNIT_SP, 21);
				title.setTextColor(titleColor);
				title.setTypeface(null, Typeface.BOLD);
				title.setGravity(Gravity.CENTER);
				...
				TextView subtitle = new TextView(act);
				subtitle.setText(LocalizedStringProvider.getInstance().get(ctx, "online_preview_dialog_subtitle"));
				subtitle.setTextSize(TypedValue.COMPLEX_UNIT_SP, 13);
				subtitle.setTextColor(hintColor);
				subtitle.setGravity(Gravity.CENTER);
				subtitle.setPadding(0, 0, 0, dp(act, 36));
				...
				TextView enableTitle = new TextView(act);
				enableTitle.setText(LocalizedStringProvider.getInstance().get(ctx, "online_preview_enable"));
				enableTitle.setTextSize(TypedValue.COMPLEX_UNIT_SP, 18);
				enableTitle.setTypeface(null, Typeface.BOLD);
				enableTitle.setTextColor(textColor);
				enableTitle.setGravity(Gravity.START);
				...
				final Switch onlineSwitch = new Switch(act);
				onlineSwitch.setText(LocalizedStringProvider.getInstance().get(ctx, "online_preview_enable"));
				onlineSwitch.setTextSize(TypedValue.COMPLEX_UNIT_SP, 15);
				onlineSwitch.setTextColor(textColor);
				onlineSwitch.setGravity(Gravity.START | Gravity.CENTER_VERTICAL);
				onlineSwitch.setChecked(getPrefBoolean(ctx, KEY_ONLINE_PREVIEW_ENABLE, false));
				if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
					int[][] states = new int[][]{new int[]{android.R.attr.state_checked},
							new int[]{-android.R.attr.state_checked}};
					int[] colors = new int[]{switchOnColor, switchOffColor};
					ColorStateList colorStateList = new ColorStateList(states, colors);
					if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
						onlineSwitch.setThumbTintList(colorStateList);
					}
					if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
						onlineSwitch.setTrackTintList(colorStateList);
					}
				}
				LinearLayout.LayoutParams switchLp = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
						ViewGroup.LayoutParams.WRAP_CONTENT);
				switchLp.bottomMargin = dp(act, 8);
				enableContainer.addView(onlineSwitch, switchLp);

				TextView switchHint = new TextView(act);
				switchHint.setText(LocalizedStringProvider.getInstance().get(ctx, "online_preview_enable_hint"));
				switchHint.setTextSize(TypedValue.COMPLEX_UNIT_SP, 12);
				switchHint.setTextColor(hintColor);
				switchHint.setGravity(Gravity.START);
				enableContainer.addView(switchHint);
				...
				TextView notesTitle = new TextView(act);
				notesTitle.setText(LocalizedStringProvider.getInstance().get(ctx, "online_preview_notes_title"));
				notesTitle.setTextSize(TypedValue.COMPLEX_UNIT_SP, 18);
				notesTitle.setTypeface(null, Typeface.BOLD);
				notesTitle.setTextColor(textColor);
				notesTitle.setGravity(Gravity.START);
				...
				TextView notesContent = new TextView(act);
				notesContent.setText(LocalizedStringProvider.getInstance().get(ctx, "online_preview_notes_content"));
				notesContent.setTextSize(TypedValue.COMPLEX_UNIT_SP, 13);
				notesContent.setTextColor(textColor);
				notesContent.setLineSpacing(dp(act, 4), 1.2f);
				notesContainer.addView(notesContent);
				...
				TextView configTitle = new TextView(act);
				configTitle.setText(LocalizedStringProvider.getInstance().get(ctx, "online_preview_config_section"));
				configTitle.setTextSize(TypedValue.COMPLEX_UNIT_SP, 18);
				configTitle.setTypeface(null, Typeface.BOLD);
				configTitle.setTextColor(textColor);
				configTitle.setGravity(Gravity.START);
				...
				TextView sourceLabel = new TextView(act);
				sourceLabel.setText(LocalizedStringProvider.getInstance().get(ctx, "online_preview_source"));
				sourceLabel.setTextColor(hintColor);
				sourceLabel.setTextSize(TypedValue.COMPLEX_UNIT_SP, 13);
				sourceLabel.setTypeface(null, Typeface.BOLD);
				sourceLabel.setPadding(0, 0, 0, dp(act, 4));
				root.addView(sourceLabel);

				final int savedSource = getPrefInt(ctx, KEY_ONLINE_PREVIEW_SOURCE, 0);
				final String[] sourceItems = {
						LocalizedStringProvider.getInstance().get(ctx, "online_preview_source_kkfileview"),
						LocalizedStringProvider.getInstance().get(ctx, "online_preview_source_basemetas")};
				final TextView sourceSelector = new TextView(act);
				sourceSelector.setText(sourceItems[savedSource]);
				sourceSelector.setTextSize(TypedValue.COMPLEX_UNIT_SP, 13);
				sourceSelector.setPadding(dp(act, 12), dp(act, 8), dp(act, 12), dp(act, 8));
				GradientDrawable sourceSelectorBg = new GradientDrawable();
				sourceSelectorBg.setColor(editBgColor);
				sourceSelectorBg.setCornerRadius(dp(act, 8));
				sourceSelectorBg.setStroke(dp(act, 1), dividerColor);
				sourceSelector.setBackground(sourceSelectorBg);
				sourceSelector.setTextColor(textColor);
				sourceSelector.setOnClickListener(new View.OnClickListener() {
					@Override
					public void onClick(View v) {
						showMonetBasePopup(ctx, sourceSelector, sourceItems, new SourceSelectedCallback() {
							@Override
							public void onSelected(int pos) {
								sourceSelector.setText(sourceItems[pos]);
								putPrefInt(ctx, KEY_ONLINE_PREVIEW_SOURCE, pos);
							}
						});
					}
				});
				LinearLayout.LayoutParams sourceSelectorLp = new LinearLayout.LayoutParams(
						ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT);
				sourceSelectorLp.bottomMargin = dp(act, 8);
				root.addView(sourceSelector, sourceSelectorLp);
				...
				LinearLayout wordContainer = createOnlinePreviewCheckbox(act, ctx,
						LocalizedStringProvider.getInstance().get(ctx, "online_preview_word"),
						LocalizedStringProvider.getInstance().get(ctx, "online_preview_word_hint"),
						KEY_ONLINE_PREVIEW_WORD, textColor, hintColor, switchOnColor, switchOffColor);
				final CheckBox wordCb = (CheckBox) wordContainer.getChildAt(0);
				LinearLayout.LayoutParams cbLp1 = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
						ViewGroup.LayoutParams.WRAP_CONTENT);
				cbLp1.bottomMargin = dp(act, 8);
				configInner.addView(wordContainer, cbLp1);

				LinearLayout pptContainer = createOnlinePreviewCheckbox(act, ctx,
						LocalizedStringProvider.getInstance().get(ctx, "online_preview_ppt"),
						LocalizedStringProvider.getInstance().get(ctx, "online_preview_ppt_hint"),
						KEY_ONLINE_PREVIEW_PPT, textColor, hintColor, switchOnColor, switchOffColor);
				final CheckBox pptCb = (CheckBox) pptContainer.getChildAt(0);
				LinearLayout.LayoutParams cbLp2 = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
						ViewGroup.LayoutParams.WRAP_CONTENT);
				cbLp2.bottomMargin = dp(act, 8);
				configInner.addView(pptContainer, cbLp2);

				LinearLayout excelContainer = createOnlinePreviewCheckbox(act, ctx,
						LocalizedStringProvider.getInstance().get(ctx, "online_preview_excel"),
						LocalizedStringProvider.getInstance().get(ctx, "online_preview_excel_hint"),
						KEY_ONLINE_PREVIEW_EXCEL, textColor, hintColor, switchOnColor, switchOffColor);
				final CheckBox excelCb = (CheckBox) excelContainer.getChildAt(0);
				LinearLayout.LayoutParams cbLp3 = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
						ViewGroup.LayoutParams.WRAP_CONTENT);
				cbLp3.bottomMargin = dp(act, 8);
				configInner.addView(excelContainer, cbLp3);

				LinearLayout pdfContainer = createOnlinePreviewCheckbox(act, ctx,
						LocalizedStringProvider.getInstance().get(ctx, "online_preview_pdf"),
						LocalizedStringProvider.getInstance().get(ctx, "online_preview_pdf_hint"),
						KEY_ONLINE_PREVIEW_PDF, textColor, hintColor, switchOnColor, switchOffColor);
				final CheckBox pdfCb = (CheckBox) pdfContainer.getChildAt(0);
				configInner.addView(pdfContainer);
				...
				onlineSwitch.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
					@Override
					public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
						if (isOnlinePreviewSwitchChanging) {
							return;
						}
						if (isChecked) {
				...
				ok.setOnClickListener(new View.OnClickListener() {
					@Override
					public void onClick(View v) {
						boolean enable = getPrefBoolean(ctx, KEY_ONLINE_PREVIEW_ENABLE, false);
						boolean wordChecked = wordCb.isChecked();
						boolean pptChecked = pptCb.isChecked();
						boolean excelChecked = excelCb.isChecked();
						boolean pdfChecked = pdfCb.isChecked();

						putPrefBoolean(ctx, KEY_ONLINE_PREVIEW_ENABLE, enable);
						putPrefBoolean(ctx, KEY_ONLINE_PREVIEW_WORD, wordChecked);
						putPrefBoolean(ctx, KEY_ONLINE_PREVIEW_PPT, pptChecked);
						putPrefBoolean(ctx, KEY_ONLINE_PREVIEW_EXCEL, excelChecked);
						putPrefBoolean(ctx, KEY_ONLINE_PREVIEW_PDF, pdfChecked);

						dialog.dismiss();
						jiguroMessage(LocalizedStringProvider.getInstance().get(ctx, "online_preview_saved"));
					}
				});
			}
		});
	...
	private void showOnlinePreviewEnableWarningDialog(final Activity act, final Context ctx,
			final Switch onlineSwitch) {
		act.runOnUiThread(new Runnable() {
			@Override
			public void run() {
				if (act.isFinishing() || act.isDestroyed())
				...
				TextView title = new TextView(act);
				title.setText(LocalizedStringProvider.getInstance().get(ctx, "online_preview_enable_warning_title"));
				title.setTextSize(TypedValue.COMPLEX_UNIT_SP, 18);
				title.setTextColor(textColor);
				title.setTypeface(null, Typeface.BOLD);
				title.setGravity(Gravity.CENTER);
				...
				message.setText(
						LocalizedStringProvider.getInstance().get(ctx, "online_preview_enable_warning_message"));
				...
				dialog.setButton(AlertDialog.BUTTON_NEGATIVE,
						...
						new DialogInterface.OnClickListener() {
							...
							public void onClick(DialogInterface dialogInterface, int i) {
								...
								isOnlinePreviewSwitchChanging = true;
								onlineSwitch.setChecked(false);
								isOnlinePreviewSwitchChanging = false;
								dialog.dismiss();
							}
						});
				...
				dialog.setButton(AlertDialog.BUTTON_POSITIVE,
						...
						new DialogInterface.OnClickListener() {
							...
							public void onClick(DialogInterface dialogInterface, int i) {
								...
								putPrefBoolean(ctx, KEY_ONLINE_PREVIEW_ENABLE, true);
								dialog.dismiss();
							}
						});
	...
	private LinearLayout createOnlinePreviewCheckbox(Activity act, Context ctx, String title, String hint,
			String prefKey, int textColor, int hintColor, int switchOnColor, int switchOffColor) {
		...
		cb.setTextColor(textColor);
		cb.setChecked(getPrefBoolean(ctx, prefKey, false));
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
			int[][] states = new int[][]{new int[]{android.R.attr.state_checked},
					new int[]{-android.R.attr.state_checked}};
			int[] colors = new int[]{switchOnColor, switchOffColor};
	...
	private void performSandboxOperation(final Context ctx, final boolean enable, final boolean forceRestore) {
		final Activity act = getActivityFrom(ctx);
		if (act == null) {
			return;
		}
		...
		if (packageName == null || packageName.isEmpty()) {
			jiguroMessageWithContext(ctx, LocalizedStringProvider.getInstance().get(ctx, "user_sandbox_no_package"));
			return;
		}
		...
		LinearLayout.LayoutParams progressLp = new LinearLayout.LayoutParams(dp(act, 200),
				ViewGroup.LayoutParams.WRAP_CONTENT);
		...
		final Runnable updateProgress = new Runnable() {
			...
			public void run() {
				if (enable) {
					title.setText(LocalizedStringProvider.getInstance().get(ctx, "user_sandbox_backup_title"));
					statusText.setText(LocalizedStringProvider.getInstance().get(ctx, "user_sandbox_backup_status"));
				} else {
					title.setText(LocalizedStringProvider.getInstance().get(ctx, "user_sandbox_restore_title"));
					statusText.setText(LocalizedStringProvider.getInstance().get(ctx, "user_sandbox_restore_status"));
				}
			}
		};
				...
				try {
					...
					if (enable) {
						...
						act.runOnUiThread(new Runnable() {
							...
							public void run() {
								...
								statusText.setText(
										LocalizedStringProvider.getInstance().get(ctx, "user_sandbox_delete_status"));
							}
						});
						...
						act.runOnUiThread(new Runnable() {
							...
							public void run() {
								...
								statusText.setText(
										LocalizedStringProvider.getInstance().get(ctx, "user_sandbox_complete"));
							}
						});
						...
						if (!backupZipFile.exists()) {
							final String errMsg = LocalizedStringProvider.getInstance().get(ctx,
									"user_sandbox_error_no_backup");
					...
					act.runOnUiThread(new Runnable() {
						...
						public void run() {
							...
							if (enable) {
								jiguroMessageWithContext(ctx, LocalizedStringProvider.getInstance().get(ctx,
										"user_sandbox_error_backup_failed"));
							} else {
								jiguroMessageWithContext(ctx, LocalizedStringProvider.getInstance().get(ctx,
										"user_sandbox_error_restore_failed"));
							}
						}
					});
				}
	...
	private void exitViaAndApply(final Context ctx) {
		...
		jiguroMessageWithContext(ctx, LocalizedStringProvider.getInstance().get(ctx, "user_sandbox_exiting_via"));
	...
	void showMonetBasePopup(final Context ctx, View anchor, String[] items, final SourceSelectedCallback callback) {
	...
	private LinearLayout createStartupImageCard(final Activity act, final Context ctx, int bgColor, int textColor,
		...
		LinearLayout.LayoutParams borderColorTitleLp = new LinearLayout.LayoutParams(
				ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT);
		...
		forceStretchContainer
				.setVisibility(StartupExecutionHelper.getStartupImageEnable(ctx) ? View.VISIBLE : View.GONE);
		...
		pickBtn.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				act.startActivityForResult(Intent.createChooser(i,
						LocalizedStringProvider.getInstance().get(ctx, "startup_image_pick_title")), 0x3001);
			}
		});

		borderColorEdit.addTextChangedListener(new TextWatcher() {
			@Override
			public void beforeTextChanged(CharSequence s, int start, int count, int after) {
			}
		...
		forceStretchSwitch.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			@Override
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
	...
	private LinearLayout createStartupMusicCard(final Activity act, final Context ctx, int bgColor, int textColor,
		...
		pickBtn.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				i.addCategory(Intent.CATEGORY_OPENABLE);
				act.startActivityForResult(Intent.createChooser(i,
						LocalizedStringProvider.getInstance().get(ctx, "startup_music_pick_title")), 0x3002);
			}
		});
	...
	private LinearLayout createSandboxCheckbox(Activity act, Context ctx, String title, String hint, String prefKey,
			int textColor, int hintColor, int switchOnColor, int switchOffColor) {
		LinearLayout container = new LinearLayout(act);
		container.setOrientation(LinearLayout.HORIZONTAL);
		container.setGravity(Gravity.CENTER_VERTICAL);
	...
	void jiguroMessageWithContext(Context ctx, String msg) {
		if (ctx == null) {
			bvLog("[BetterVia] Context为null，无法显示Toast: " + msg);
			return;
		}
	...
	public static String getNetworkSource(Context ctx) {
		try {
			Object sp = XposedHelpers.callMethod(ctx, "getSharedPreferences", "BetterVia", Context.MODE_PRIVATE);
			String source = (String) XposedHelpers.callMethod(sp, "getString", KEY_NETWORK_SOURCE,
	...
	private void checkUpdate(final Context ctx) {
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					String networkSource = getPrefString(ctx, KEY_NETWORK_SOURCE, DEFAULT_NETWORK_SOURCE);
					String updateUrl = networkSource.equals(NETWORK_SOURCE_VERCEL)
							? VERCEL_UPDATE_URL
							: GITHUB_UPDATE_URL;
	...
	private void showAboutDialog(final Context ctx, final AlertDialog mainDialog) {
		final Activity act = getActivityFrom(ctx);
		if (act == null)
			return;
		...
		act.runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				addClickableAboutItem(root, act, LocalizedStringProvider.getInstance().get(ctx, "about_blog"),
						LocalizedStringProvider.getInstance().get(ctx, "about_blog_url"), new View.OnClickListener() {
							@Override
							public void onClick(View v) {
								openUrl(act, "https://www.196104.xyz");
								jiguroMessageWithContext(act,
										LocalizedStringProvider.getInstance().get(ctx, "start_url_message"));
								dialog.dismiss();
								if (mainDialog != null && mainDialog.isShowing()) {
				...
				addClickableAboutItem(root, act, "", "Coolapk @半烟半雨溪桥畔", new View.OnClickListener() {
					@Override
					public void onClick(View v) {
						openUrl(act, "https://gitee.com/daybreak166");
						jiguroMessageWithContext(act,
								LocalizedStringProvider.getInstance().get(ctx, "start_url_message"));
						dialog.dismiss();
						if (mainDialog != null && mainDialog.isShowing()) {
				...
				addClickableAboutItem(root, act, "", "Blog @sgfox", new View.OnClickListener() {
					@Override
					public void onClick(View v) {
						openUrl(act, "https://www.sgfox.cc");
						jiguroMessageWithContext(act,
								LocalizedStringProvider.getInstance().get(ctx, "start_url_message"));
						dialog.dismiss();
						if (mainDialog != null && mainDialog.isShowing()) {
				...
				addClickableAboutItem(root, act, "", "Github @ECHSFXY", new View.OnClickListener() {
					@Override
					public void onClick(View v) {
						openUrl(act, "https://github.com/echs-top");
						jiguroMessageWithContext(act,
								LocalizedStringProvider.getInstance().get(ctx, "start_url_message"));
						dialog.dismiss();
						if (mainDialog != null && mainDialog.isShowing()) {
	...
	private void showShisuiDialog(final Context ctx) {
		...
		act.runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				String savedSource = getPrefString(ctx, KEY_NETWORK_SOURCE, DEFAULT_NETWORK_SOURCE);
				final String shisuiUrl = savedSource.equals(NETWORK_SOURCE_VERCEL)
	...
	private void showBasicSettingsDialog(final Context ctx) {
		...
		act.runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				langSectionTitle
						.setText(LocalizedStringProvider.getInstance().get(ctx, "basic_settings_language_section"));
				...
				networkSectionTitle
						.setText(LocalizedStringProvider.getInstance().get(ctx, "basic_settings_network_section"));
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					...
					String networkSource = getPrefString(ctx, KEY_NETWORK_SOURCE, DEFAULT_NETWORK_SOURCE);
					String jsonUrl = NETWORK_SOURCE_VERCEL.equals(networkSource)
					...
					public void onClick(View v) {
						...
						MappingSyncHelper.syncMappingFromNetwork(ctx, new MappingSyncHelper.SyncCallback() {
							...
							public void onSuccess(int addedVersions) {
								...
								syncButton
										.setText(LocalizedStringProvider.getInstance().get(ctx, "mapping_sync_title"));
								...
								putPrefBoolean(ctx, KEY_SKIP_CLEAR_NETWORK_MAPPINGS, true);
								bvLog("[BetterVia] 同步成功，设置跳过清理标记以保留新映射");
			...
			finalActivity.runOnUiThread(new Runnable() {
				...
				public void run() {
					...
					try {
						...
						TextView titleView = new TextView(finalActivity);
						titleView.setText(LocalizedStringProvider.getInstance().get(ctx, "version_error_title"));
						titleView.setTextColor(Color.WHITE);
						titleView.setTextSize(TypedValue.COMPLEX_UNIT_SP, 14);
						titleView.setTypeface(null, Typeface.BOLD);
						titleView.setPadding(dp(finalActivity, 16), dp(finalActivity, 12), dp(finalActivity, 16),
						...
						hintView.setText(LocalizedStringProvider.getInstance().get(ctx, "version_error_hint"));
						...
						cleaningView.setText(LocalizedStringProvider.getInstance().get(ctx, "version_error_cleaning"));
						cleaningView.setTextColor(Color.GREEN);
						cleaningView.setTextSize(TypedValue.COMPLEX_UNIT_SP, 11);
						cleaningView.setPadding(dp(finalActivity, 16), dp(finalActivity, 8), dp(finalActivity, 16),
								dp(finalActivity, 12));
	...
	private void performVersionErrorClean(final Activity act) {
		...
		final AlertDialog loadingDialog = new AlertDialog.Builder(act)
				.setTitle(LocalizedStringProvider.getInstance().get(act, "version_error_cleaning")).setView(progressBar)
				.setCancelable(false).create();
	...
	public static int getTitleColorStatic(Context ctx) {
		boolean isDark = isDarkTheme(ctx);
		return isDark ? DARK_TITLE_COLOR : LIGHT_TITLE_COLOR;
	}
	...
	public static int getHintColorStatic(Context ctx) {
		boolean isDark = isDarkTheme(ctx);
		return isDark ? DARK_HINT_COLOR : LIGHT_HINT_COLOR;
	}

	public static int getDividerColorStatic(Context ctx) {
		boolean isDark = isDarkTheme(ctx);
		return isDark ? DARK_DIVIDER_COLOR : LIGHT_DIVIDER_COLOR;
	}

	public static int getBtnBgColorStatic(Context ctx) {
		boolean isDark = isDarkTheme(ctx);
		return isDark ? DARK_BTN_BG_COLOR : LIGHT_BTN_BG_COLOR;
	}

	public static int getBtnTextColorStatic(Context ctx) {
		boolean isDark = isDarkTheme(ctx);
		return isDark ? DARK_BTN_TEXT_COLOR : LIGHT_BTN_TEXT_COLOR;
	}

	public static int getItemBgColorStatic(Context ctx) {
		boolean isDark = isDarkTheme(ctx);
		return isDark ? DARK_ITEM_BG_COLOR : LIGHT_ITEM_BG_COLOR;
	}

	public static int getOkBtnBgColorStatic(Context ctx) {
		boolean isDark = isDarkTheme(ctx);
		return isDark ? DARK_OK_BTN_BG_COLOR : LIGHT_OK_BTN_BG_COLOR;
	}

	public static int getOkBtnTextColorStatic(Context ctx) {
		boolean isDark = isDarkTheme(ctx);
		return isDark ? DARK_OK_BTN_TEXT_COLOR : LIGHT_OK_BTN_TEXT_COLOR;
	}

	public static boolean isDarkTheme(Context ctx) {
		String theme = getPrefStringStatic(ctx, KEY_MODULE_THEME, DEFAULT_MODULE_THEME);
		if (MODULE_THEME_DARK.equals(theme)) {
			return true;
		} else if (MODULE_THEME_LIGHT.equals(theme)) {
			return false;
		} else {
